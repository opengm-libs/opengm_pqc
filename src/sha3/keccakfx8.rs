...
#[inline]
pub(crate) fn keccak_f1600_x8(
    a: &mut [u64; 25],b: &mut [u64; 25],c: &mut [u64; 25],d: &mut [u64; 25],
    e: &mut [u64; 25],f: &mut [u64; 25],g: &mut [u64; 25],h: &mut [u64; 25],
) {
    unsafe { keccak_f1600_generic_x8(a,b,c,d,e,f,g,h) };
}
...
macro_rules! round {
    (
        ...
        $a0 = bc0 ^ (bc2 & !bc1) ^ u64x8::splat($rci);
        $a6 = bc1 ^ (bc3 & !bc2);
        $a12 = bc2 ^ (bc4 & !bc3);
        $a18 = bc3 ^ (bc0 & !bc4);
        $a24 = bc4 ^ (bc1 & !bc0);
...
pub(crate) fn keccak_f1600_generic_x8(
    a: &mut [u64; 25],b: &mut [u64; 25],c: &mut [u64; 25],d: &mut [u64; 25],
    e: &mut [u64; 25],f: &mut [u64; 25],g: &mut [u64; 25],h: &mut [u64; 25]
) {
    let mut i = 0;
    let mut v:[u64x8;25] = [u64x8::default();25];

    // read to v
    for i in 0..25{
        v[i] = Simd::from_array([a[i], b[i], c[i], d[i],e[i], f[i], g[i],h[i]]);
    }
    ...
    // write back
    for i in 0..25{
        [a[i], b[i], c[i], d[i],e[i], f[i], g[i],h[i]] = v[i].to_array();
    }
}
...
mod tests {
    ...
    fn test_f1600_x8() {
        ...
        unsafe { keccak_f1600_generic_x8(&mut a, &mut b, &mut c, &mut d, &mut e, &mut f, &mut g, &mut h) };
    ...
    fn bench_f1600(b: &mut Bencher) {
        ...
        let mut a4:[u64;25] = rng.random();
        let mut a5:[u64;25] = rng.random();
        let mut a6:[u64;25] = rng.random();
        let mut a7:[u64;25] = rng.random();
        b.iter(|| {
            test::black_box({
                unsafe { keccak_f1600_generic_x8(&mut a0, &mut a1, &mut a2, &mut a3, &mut a4,&mut a5,&mut a6,&mut a7) };
            });
        });
    }
}
